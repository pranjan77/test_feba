...
function fitblast_short(row,server_root) {
    ...
    var sigstring = hasstrong ? "strong phenotype" : (hassig? "phenotype" : (row.minfit !== "" ? "has data" : "no data"));
    if (row.minfit !== "") {
...
function fitblast_load_short(id, server_root, sequence) {
	...
	for (i = 0; i < data.length; i++) {
	    ...
	    //console.log(row.organism + " " + (useful?"useful":"not"));
...
function fitblast_load_table(id, server_root, sequence) {
	...
	var cells = rows.selectAll("td").data(function(row) {
		...
		if (row.minfit !== "") {
		    var colLo = row.minfit < -2 ? "#0000FF" : (row.minfit < -1 ? "#7777FF" : "#777777");
		    var colHi = row.maxfit > 2 ? "#0000FF" : (row.maxfit > 1 ? "#7777FF" : "#777777");
		    showFit = '<small><div style="color:' + colLo + '">' + (+row.minfit).toFixed(1)
			+ '</div> <div style="color:' + colHi + '">' + (+row.maxfit).toFixed(1)
			+ '</div></small>';
		...
		var show = { Organism: row.organism,
			     ...
			     Identity: '<small>' + row.identity + "%</small>",
