...
{
  my $minFraction = 0.2;
  my $minCount = 200 * 1000;
  my $usage = <<END
Usage: combineBarSeq.pl [ -all ] out pool_file codesfiles...
  Codes files may be gzipped.
  Writes out.poolcount and out.colsum
  By default, experiments with few reads matching the pool are omitted from
    the output table; use -all to reverse this;

These options define a successful sample:
  -minFraction $minFraction: minimum fraction of reads to match the pool
  -minCount $minCount: minimum counts for a successful sample
END
;
  my $all;
  die $usage
    unless GetOptions('all' => \$all, 'minFraction=f' => \$minFraction, 'minCount=i' => \$minCount)
      && @ARGV >= 3;
  my $out = shift @ARGV;
  ...
  # categorize reads
  my @lowcountI = ();           # indexes with low reads
  my @lowcounts = ();           # those actual counts
  my @fractions = (); # fraction used for indexes with plenty of reads only
  my @fractionsS = (); # fraction used for successful indexes (enough reads and f >= minFraction)
  my @lowhitI = ();    # indexes with low fraction
  my @okI = ();    # indexes with enough reads and high fraction
  my @iOut; # index into indexes (or counts) of those successful samples
  my $totalReads = 0;
  foreach my $i (0..(scalar(@indexes)-1)) {
    $totalReads += $colSums[$i];
    if ($colSums[$i] < $minCount) {
  ...
  print COUNT join("\t", "barcode", "rcbarcode", "scaffold", "strand", "pos", @indexOut)."\n";
  ...
  foreach my $rcbarcode (@sorted) {
    ...
    if (defined $counts) {
      ...
      push @out, (0) x (defined $all ? $nSamples : scalar(@okI));
    }
    print COUNT join("\t", @out)."\n";
  }
  close(COUNT) || die "Error writing to $out.poolcount";
  if (defined $all) {
    print STDERR "  Wrote all $nSamples samples to $out.poolcount\n";
  } else {
    print STDERR "  Wrote " . scalar(@okI) . " successful samples to $out.poolcount\n";
  }
