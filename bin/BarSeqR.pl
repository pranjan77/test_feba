...

{
  ...
  while (my ($set, $filelist) = each %setFiles) {
    foreach my $file (@$filelist) {
      ...
      die "Cannot read $indir/$file.poolcount\n" unless defined $fh;
      push @{ $setFh{$set} }, $fh;
      ...
      if (!exists $setIndex{$set}) {
        ...
        foreach my $i (0..(scalar(@expect)-1)) {
          ...
          die "Expected $expect[$i] from first file but see $field in $indir/$file.poolcount -- you may need to rerun combineBarSeq.pl with -all for both\n"
