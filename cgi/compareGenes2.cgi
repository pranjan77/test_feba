...
if (defined $query1 && $query1 ne "") {
    ...
    @genes = grep { $_->{locusId} ne $locus2 } @exps if @genes > 1;
    if (@genes == 1) {
    ...
    my @exps = @{ Utils::matching_genes($dbh,$orgId,$query2) };
    Utils::fail($cgi, qq{No genes matching "$query2"}) if @genes == 0;
    @genes = grep { $_->{locusId} ne $locus1 } @exps if @genes > 1;
    if (@genes == 1) {
	$gene2 = $exps[0]{locusId};
    } else {
	@geneCand = @genes;
	$choosing = 2;
    }
...
#     my $geneConst = $choosing == 1 ? $locus2 : $locus1;
#     my $geneConst = $dbh->selectrow_hashref("SELECT * from Gene WHERE orgId = ? AND locusId = ?",
# 					   {}, $orgId, $geneConst);
#     die "Unknown locus: $geneConst" unless exists $geneConst->{locusId};
#     my $notChoosing = $choosing == 1 ? 2 : 1;

#     my @trows = ();
#     my @headings = qq{'&nbsp;', 'Gene ID', name group condition description};
#     push @trows, $cgi->Tr({-valign => 'top', -align => 'center'}, $cgi->th(\@headings));
#     my $isFirst = 1;
#     foreach my $exp (@genepCand) {
...
if ($tsv || $outlier) {
    ...
    foreach my $row (@$fit) { 
      $exps = $dbh->selectall_hashref("SELECT * FROM Experiment where orgId = ? and expName = ?", "locusId", {}, $orgId, $fit->{expName});
    }
