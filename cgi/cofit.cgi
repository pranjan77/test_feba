...
my $orgId = $cgi->param('species') || die "no species identifier\n";
...
my ($sysName,$geneName,$desc) = $dbh->selectrow_array("SELECT sysName,gene,desc FROM Gene WHERE nickname=? AND locusId=?", undef,
					    $orgId, $locusId);
Utils::fail($cgi, "No locus $locusId in species $orgId") unless defined $sysName;
my ($genus,$species,$strain) = $dbh->selectrow_array("SELECT genus,species,strain FROM Organism WHERE name=?", undef, $orgId);
...
#print $cgi->p(qq{<A HREF="myFitShow.cgi?species=$orgId&gene=$locusId">$sysName: $desc</A>});
...
if (@$cofitResults == 0) {
	...
	push @trows, $cgi->Tr({bgcolor => $rowcolor, align => 'left', valign => 'top' },
			      $cgi->td($rank),
			      $cgi->td($cofit),
			      $cgi->td($cgi->a( {href => "myFitShow.cgi?species=$orgId&gene=$hitId" },
						$showId )),
			      $cgi->td($hitName),
			      $cgi->td($hitDesc));
    }
...
print $cgi->h4($cgi->a({href => "myFitShow.cgi?species=$orgId&gene=$locusId"}, "Fitness data"));
print $cgi->h4($cgi->a({href => "mySeqSearch.cgi?species=$orgId&gene=$locusId"}, "Check homologs"));
